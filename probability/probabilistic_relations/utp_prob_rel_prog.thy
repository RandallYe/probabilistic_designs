...
(*
definition pcomp :: "('s\<^sub>1, 's\<^sub>2) prel \<Rightarrow> ('s\<^sub>1, 's\<^sub>2) prel \<Rightarrow> ('s\<^sub>1, 's\<^sub>2) prel" (infixl ";\<^sub>p" 163) where
[prob_rel_defs]: "pcomp P Q = "
*)

definition pparallel :: "('s\<^sub>1, 's\<^sub>2) prel \<Rightarrow> ('s\<^sub>1, 's\<^sub>2) prel \<Rightarrow> ('s\<^sub>1, 's\<^sub>2) prel" 
  (infixl "\<parallel>\<^sub>p" 166) where
[prob_rel_defs]: "pparallel P Q = (P * Q)\<^sub>e"
